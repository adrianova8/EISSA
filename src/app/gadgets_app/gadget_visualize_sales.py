...
def sales_visualizer_container(app: Tk, app_window: AppWindow, frame_manager: FrameManager) -> None:
    ...
    def make_total_frame(label, color=METAL_GOLD_COLOR):
        ...
        app_window.create_label(f, text=label, font=("Times New Roman", 16, "bold"),
                                bg=BROWN_COLOR, fg=color)
        lbl = app_window.create_label(f, text="1.00 €", font=("Times New Roman", 16),
                                      bg=BEIGE_COLOR, fg=BLACK_COLOR)
    ...
    canvas_container = app_window.create_frame(sales_list_frame, bg=BEIGE_COLOR)
    ...
    for text, cmd in [
        ...
        tk.Button(nav_frame, text=text, font=("Times New Roman", 11, "bold"),
                  ...
                  command=cmd, relief="raised", bd=2, width=10).pack(side="left", padx=3)
...
def add_sale_to_visualizer(frame_manager: FrameManager, amount: float, payment_method: str):
    ...
    sale_text = f"{time_now} - {amount:.3f}€ ({payment_method})"
    tk.Label(
        sale_frame,
        text=sale_text,
    ...
    if payment_method == "Efectiu":
        current_cash += amount
        cash_total.config(text=f"{current_cash:.3f} €")
    else:
        current_card += amount
        card_total.config(text=f"{current_card:.3f} €")

    total_sales.config(text=f"{(current_cash + current_card):.3f} €")
