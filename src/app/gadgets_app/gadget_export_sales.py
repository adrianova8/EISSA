from src.utils.common.names import (
    METAL_GOLD_COLOR, BEIGE_COLOR, BLACK_COLOR, WHITE_COLOR)
    ...
    try:
        ...
        if not export_dir:
            messagebox.showerror(
                "Error", "No s'ha pogut crear el directori d'exportació")
            return False
        ...
        messagebox.showinfo(
            "Èxit", f"Arxiu Excel creat correctament!\nRuta: {full_path}")
        logger.info(f"Arxiu Excel creat exitosament: {full_path}")
        return True

    except Exception as e:
        messagebox.showerror(
            "Error", f"Error al exportar l'arxiu Excel:\n{str(e)}")
